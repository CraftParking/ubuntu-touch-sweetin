...
class Service(Object):
    ...
    @log_and_exit
    @method('com.canonical.SystemImage')
    def CraftparkingApplyUpdate(self):
        """Install the downloaded CraftParking flashable zip via TWRP, then reboot."""
        self._api.craftparking_apply_update()

    @log_and_exit
    @method('com.canonical.SystemImage', in_signature='ssx', out_signature='b')
    def CraftparkingStartDownload(self, url, sha256, size):
        """Start (or no-op if already running) the background update download.

        Runs inside this service, not the caller's process, so it survives
        the Settings app being backgrounded, navigated away from, or closed
        entirely - only cancelling or finishing actually stops it.
        """
        self.loop.keepalive()
        started = craftparking_download.start(
            url, sha256, size,
            lambda received, total: GLib.idle_add(
                self._craftparking_on_progress, received, total),
            lambda success, error: GLib.idle_add(
                self._craftparking_on_finished, success, error))
        return started

    def _craftparking_on_progress(self, received, total):
        self.loop.keepalive()
        self.CraftparkingDownloadProgress(received, total)
        return False

    def _craftparking_on_finished(self, success, error):
        self.CraftparkingDownloadFinished(success, error)
        return False

    @log_and_exit
    @method('com.canonical.SystemImage')
    def CraftparkingCancelDownload(self):
        craftparking_download.cancel()

    @log_and_exit
    @method('com.canonical.SystemImage', out_signature='a{sv}')
    def CraftparkingDownloadStatus(self):
        """Lets a (re)opened Settings page resync with an in-flight download."""
        status = craftparking_download.status()
        # Plain Python ints default to 32-bit ('i') in a{sv} - our own zip
        # is already close to that limit (~2GB) and will exceed it as this
        # ROM grows, so received/total need to be explicitly 64-bit.
        status['received'] = dbus.Int64(status['received'])
        status['total'] = dbus.Int64(status['total'])
        return status

    @log_and_exit
    @signal('com.canonical.SystemImage', signature='xx')
    def CraftparkingDownloadProgress(self, received, total):
        pass

    @log_and_exit
    @signal('com.canonical.SystemImage', signature='bs')
    def CraftparkingDownloadFinished(self, success, error):
        pass
