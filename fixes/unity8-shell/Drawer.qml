...
FocusScope {
    ...
    Rectangle {
        ...
        // long-press menu for drawer icons: open / pin-unpin / go to store
        Component {
            id: appActionsPopoverComponent
            ActionSelectionPopover {
                id: appActionsPopover
                property string popoverAppId: ""
                property bool isStoreApp: false
                contentWidth: units.gu(22)
                // bigger than default so top-row icons don't pop up into the camera cutout
                edgeMargins: units.gu(5)

                Timer {
                    // auto-dismiss so a forgotten popup can't linger over the lock screen
                    interval: 6000
                    running: true
                    onTriggered: PopupUtils.close(appActionsPopover)
                }

                function appIsPinned(appId) {
                    for (var i = 0; i < LauncherModel.count; i++) {
                        var item = LauncherModel.get(i);
                        if (item.appId === appId && item.pinned) {
                            return true;
                        }
                    }
                    return false;
                ...
                actions: ActionList {
                    Action {
                        text: i18n.tr("Open")
                        onTriggered: root.applicationSelected(appActionsPopover.popoverAppId)
                    }
                    Action {
                        text: appActionsPopover.appIsPinned(appActionsPopover.popoverAppId) ?
                                  i18n.tr("Unpin") : i18n.tr("Pin")
                        onTriggered: {
                            if (appActionsPopover.appIsPinned(appActionsPopover.popoverAppId)) {
                                LauncherModel.requestRemove(appActionsPopover.popoverAppId);
                            } else {
                                LauncherModel.pin(appActionsPopover.popoverAppId);
                            }
                        }
                    }
                    Action {
                        text: i18n.tr("Go to Store")
                        visible: appActionsPopover.isStoreApp
                        onTriggered: {
                            var splitAppId = appActionsPopover.popoverAppId.split("_");
                            Qt.openUrlExternally("https://open-store.io/app/" +
                                appActionsPopover.popoverAppId.replace("_" + splitAppId[splitAppId.length-1],"") + "/");
                        }
                    }
                }
            }
        ...
        Component {
            ...
            AbstractButton {
                ...
                onPressAndHold: {
                    PopupUtils.open(appActionsPopoverComponent, drawerDelegate, {
                        "popoverAppId": model.appId,
                        "isStoreApp": model.appId.includes(".")
                    });
                }
                z: loader.active ? 1 : 0
