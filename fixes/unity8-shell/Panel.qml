...
Item {
    ...
    Item {
        ...
        // HAX: notifications (indicator-messages) get their own row on the
        // left, clear of the center punch-hole camera; hidden from the main
        // (now system-only) indicators row via hideMessagesIndicator below.
        Row {
            id: notificationRow
            ...
            Repeater {
                ...
                delegate: IndicatorItem {
                    ...
                    busName: model.indicatorProperties.busName
                    actionsObjectPath: model.indicatorProperties.actionsObjectPath
                    menuObjectPath: model.indicatorProperties.menuObjectPath
                    height: notificationRow.height
                    expanded: false
                    visible: identifier == "indicator-messages" && indicatorVisible
        ...
        PanelMenu {
            ...
            // HAX: half-width of the camera-cutout clearance zone, dead
            // center of the bar - used below by hideCameraOverlap, kept
            // separate from overFlowWidth since that also sizes
            // __showDragHandle (the invisible tap-to-open-quick-settings
            // area) and must stay full-width for that to keep working
            // across the whole bar.
            property real cameraSafeHalfWidth: units.gu(4)
            ...
            rowItemDelegate: IndicatorItem {
                ...
                readonly property bool hidden: !expanded && (overflow || !indicatorVisible || hideSessionIndicator || hideKeyboardIndicator || hideMessagesIndicator || hideCameraOverlap)
                ...
                // for right-alignment) - not the screen width, so it can't
                // be compared against a screen-scale threshold directly.
                // mapToItem gives a real absolute position regardless of
                // that internal layout complexity - but its internal
                // ancestor traversal isn't tracked by QML's binding engine,
                // so it never re-evaluates on its own once the real layout
                // settles (confirmed via logging: every icon stayed stuck
                // at its initial pre-layout x=1021,width=0 value). Forcing
                // a dependency on the properties that actually change
                // during layout (x, width, parent.width, rowContentX) makes
                // it reevaluate correctly. Full interval-overlap test (not
                // just a single edge) against the safe zone.
                readonly property real absoluteX: {
                    var _dep1 = x;
                    var _dep2 = width;
                    var _dep3 = parent.width;
                    var _dep4 = __indicators.rowContentX;
                    return mapToItem(root, 0, 0).x;
                }
