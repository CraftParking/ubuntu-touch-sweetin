                     ...
                     const char                         * object_path)
{
  ...
  if (g_str_has_suffix(object_path, "batt_therm"))
    return;
  // Craftparking (sweet/sweetin): batt_verify is a Maxim DS28E16 battery-authentication chip,
  // not a real battery - it always reports 0%/empty and gets picked over the real battery.
  if (g_str_has_suffix(object_path, "batt_verify"))
    return;
  priv_t * p = get_priv(self);
